...
export default (state: NoteState = initialState, action): NoteState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_NOTES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_NOTES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_NOTES):
...
const apiSearchUrl = 'api/_search/notes';

// Actions

export const getSearchEntities: ICrudSearchAction<INote> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_NOTES,
  payload: axios.get<INote>(`${apiSearchUrl}?query=${query}`),
});
